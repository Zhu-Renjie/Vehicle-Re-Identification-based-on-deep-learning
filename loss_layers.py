...
class VerfiLoss(mx.operator.CustomOp):
    ...
    def forward(self, is_train, req, in_data, out_data, aux):
        ...
        for i in range(x.shape[0]):
            #print x#64*1024 array
            #print n#64
            '''
            print label  #64 array
            print label[i]
            print label == label[i]
            print np.where(label == label[i]) 
            '''
            mask = np.zeros((n, ))
            #print '&&&&&&&&&&&&&&&&&&&&&&&&'
            #print mask
            mask[(np.where(label == label[i]))[0]] = 1##[0]may cause error
            pos = np.sum(mask)
    ...
    def backward(self, req, out_grad, in_data, out_data, in_grad, aux):
        ...
        for i in range(x.shape[0]):
            ...
            #print mask
            #print '>>>>>>>>>>>>>>>>>>>>>>>'
            zies=np.squeeze(np.where(label == label[i]))
            #print np.where(label == label[i])
            mask[0,(np.where(label == label[i]))] = 1#mask add 0,
            pos = np.sum(mask)
            mask = mx.nd.array(mask, ctx=ctx)
            diff = x[i] - x
            d = mx.nd.sqrt(mx.nd.sum(diff * diff, axis=1))
